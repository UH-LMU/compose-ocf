...
query = session.query(login, account_name,  account_id) \
.join(accounts, accounts.account_id==user_accounts_x.account_id)
#.having(func.count(account)>1)
result = query.all()
print_csv("/output/users_with_multiple_accounts.csv", result)
